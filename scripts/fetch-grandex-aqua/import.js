// Импорт товаров grandex-aqua из products-data.jsonl в PostgreSQL.
...
//
// Запуск (5 тестовых):
//   node scripts/fetch-grandex-aqua/import.js --limit 5
// Запуск (все):
//   node scripts/fetch-grandex-aqua/import.js
  ...
  try {
    ...
    for (const p of subset) {
      try {
        ...
        const raw = {
          parent_id: p.raw?.parent_id,
          active_offer_id: p.raw?.active_offer_id,
          color_prop_id: p.raw?.color_prop_id,
          color_onevalue: p.raw?.color_onevalue,
          color: p.raw?.color,
