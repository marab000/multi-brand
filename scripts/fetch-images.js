...
async function getImages(page, url) {
	...
	return await page.$$eval('.item_slider ul li', lis =>
		lis
			.filter(li => li.id && li.id.startsWith('photo-'))
...
async function processProduct(page, p) {
	...
	if (!images.length) {
		url = await resolveSearch(page, p.name)
		if (url) images = await getImages(page, url)
	}

	if (!images.length) {
		await log('NO IMAGES', p.name)
		return
	}
	...
	for (const imgUrl of images) {
		...
		await sql`
			...
			values(${id},${p.id},${'/images/' + b + '/fetch/' + file},'fetch',${imgUrl},${pos})
		`
	...
	if (MODE === 'missing') {
		products = await sql`
			...
			where not exists(
				...
				where pi.product_id=p.id and pi.source='fetch'
			)
			order by p.name
		`
	} else {
