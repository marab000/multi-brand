// Скачивает картинки для товаров rusklimat из image-queue.json,
...
//
// Запуск (5 тестовых):
//   node scripts/fetch-rusklimat/fetch-images.js --limit 5
// Запуск (все):
//   node scripts/fetch-rusklimat/fetch-images.js
...
// --no-db  — не подключаться к БД (пропустить проверку существующих, загрузить все; оригиналы перезапишутся)
...
function filterImageUrls(urls) {
  ...
  for (const url of urls) {
    ...
    //    Для rusklimat URL-ы вида /products/{uuid}/src.jpg — имя файла одинаковое,
    //    поэтому берём последние 2 сегмента пути (parent + filename).
    //    Для tetrasis (resize_cache) это тоже корректно — разные resize дают разный parent.
    let sig
    try {
      const u = new URL(url)
      const parts = u.pathname.split('/')
      sig = parts.slice(-2).join('/').toLowerCase()
    } catch {
      const segs = url.split('/')
      sig = segs.slice(-2).join('/').split('?')[0].toLowerCase()
    }
    if (!sig) continue
...
async function main() {
  ...
  try {
    ...
    await logInfo('START', `mode:${MODE}`, `no-db:${NO_DB}`, `queue:${queue.length}`, `processing:${subset.length}`, `limit:${LIMIT || 'all'}`)

    // Загружаем product_id из БД для всех external_id из subset
    let byExt = new Map()
    if (NO_DB) {
      // Без БД — используем external_id как product_id (запишем только в S3, лог с путями)
      for (const item of subset) byExt.set(item.external_id, { id: item.external_id, external_id: item.external_id, name: item.name })
      await logInfo('NO_DB_MODE', 'will upload to S3 only, no product_images writes')
    } else {
      ...
      byExt = new Map(products.map(p => [p.external_id, p]))
    ...
    for (let i = 0; i < subset.length; i++) {
      ...
      // В режиме missing — проверяем, есть ли уже fetch-картинки (пропускаем при --no-db)
      if (MODE === 'missing' && !NO_DB) {
      ...
      try {
        ...
        const deleted = (MODE === 'full' && !NO_DB) ? await replaceProductImages(p.id, rows) : (NO_DB ? 0 : (await saveMissingImages(rows), 0))
